...
class ModelCatalogReview extends Model {
	...
	public function editReview($review_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "review SET author = '" . $this->db->escape($data['author']) . "', product_id = '" . (int)$data['product_id'] . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', rating = '" . (int)$data['rating'] . "', status = '" . (int)$data['status'] . "', date_added = '".$data['date_added']."', date_modified = NOW() WHERE review_id = '" . (int)$review_id . "'");
		$this->cache->delete('product');
	...
	{
	    $query = $this->db->query("SELECT * FROM `".DB_PREFIX."review_image` WHERE `review_id` = '".(int)$reviewId."'");
	    return $query->rows;
	}
